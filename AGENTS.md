...
APK Deck 是一个 Android Studio / IntelliJ IDEA 插件，用于扫描当前 Android 工程中的 application 模块，检测这些应用在已连接 ADB 设备上的安装状态，并提供重新安装、清数据、卸载和系统 APK 推送等操作。

插件入口为 IntelliJ Action，主要流程为：

```text
OpenApkDeckAction
  -> ApplicationModuleScanner.scan()
  -> AdbService.getConnectedDevices()
  -> ApkDeckDialog.show()
```

核心代码位于 `src/main/kotlin/dev/apkdeck/`，图标资源位于 `src/main/resources/icons/`。
...
- `src/main/kotlin/dev/apkdeck/ApkDeckDialog.kt` 中的 `PLUGIN_VERSION` 常量
...
- `ApkDeckDialog` 包含主要 UI、事件处理和表格渲染逻辑，修改渲染或点击区域时需要同时检查视觉反馈和点击判定。
- `AppInstallationTableModel` 只展示工程 application 模块，并按模块名称稳定排序。
